        ...
        <div className="w-full h-full flex flex-col-reverse justify-center items-center lg:grid lg:grid-cols-3 gap-5 lg:gap-0">
            <div className="w-full col-span-2 flex flex-col justify-center items-center dark:text-dark-accent">
                <div className="w-full md:w-[35rem] lg:w-[60%] lg:px-5 flex flex-col justify-center items-center gap-10 border-[2px] border-black/5 dark:border-neutral-light/5 rounded-md p-4 md:shadow-md dark:shadow-neutral-light/20">
                    ...
                    <form onSubmit={handleSubmit} className="w-full flex flex-col gap-5">
                        <input type="text" name="name" id="name" value={name} placeholder="Fullname" className="w-full input input-bordered input-md focus-within:border-dark-accent/50 rounded-full dark:focus-within:outline-dark-accent dark:border-dark-accent/50 dark:placeholder:text-dark-accent dark:bg-black/50" onChange={(e)=>setName(e.target.value)}/>
                        <input type="text" name="username" id="username" value={u_name} placeholder="Username" className="w-full input input-bordered input-md focus-within:border-dark-accent/50 rounded-full dark:focus-within:outline-dark-accent dark:border-dark-accent/50 dark:placeholder:text-dark-accent dark:bg-black/50" onChange={(e)=>setUname(e.target.value)}/>
                        <input type="email" name="email" id="email" value={email} placeholder="Email" className="w-full input input-bordered input-md focus-within:border-dark-accent/50 rounded-full dark:focus-within:outline-dark-accent dark:border-dark-accent/50 dark:placeholder:text-dark-accent dark:bg-black/50" onChange={(e)=>setEmail(e.target.value)}/>
                         <select className="select select-bordered w-full input input-md focus-within:border-dark-accent/50 rounded-full dark:focus-within:outline-dark-accent dark:border-dark-accent/50 dark:placeholder:text-dark-accent dark:bg-black/50" defaultValue={"Gender?"} onChange={(e) => setGender(e.target.value)}>
                            <option disabled>Gender?</option>
                            <option>Male</option>
                            <option>Female</option>
                        </select>
                        <input type="date" name="dob" id="dob" max={getTodayDate()}
                         value={dob} placeholder="Date Of Birth" className="w-full input input-bordered input-md focus-within:border-dark-accent/50 rounded-full dark:focus-within:outline-dark-accent dark:border-dark-accent/50 dark:placeholder:text-dark-accent dark:bg-black/50" onChange={(e)=>setDob(e.target.value)}/>
                        <label className="w-full flex gap-2 justify-between items-center input input-bordered input-md focus-within:border-dark-accent/50 rounded-full dark:focus-within:outline-dark-accent dark:border-dark-accent/50 dark:placeholder:text-dark-accent dark:bg-black/50">
            ...
            <div className="w-full lg:h-full flex flex-col justify-center items-center border-l-[1px] border-neutral-100 dark:border-neutral-light/10 col-span-1 overflow-clip">
                ...
                <ul className="mt-10 gap-4 hidden lg:block">
                  <Link to={'/login'}><li className="py-2 px-4 border-[1px] rounded-full border-black text-neutral dark:border-white dark:text-dark-accent  hover:bg-black hover:text-base-100 dark:hover:bg-white">Login</li></Link>
                </ul>
            </div>
        </div>
    </div>
